...
from models.detector_Vote2Cap_DETRv2.diffusion import Diffusion
...
class Model_Vote2Cap_DETR(nn.Module):
    ...
    def __init__(
        ...
        mlp_dropout=0.3,
        num_queries=256,
        ckpt="./pretrained/diffusion/AE_scanrefer.pt",
        latent_dim = 256,
        criterion=None,
        is_pretrain=True
    ):
    ...
    def run_encoder(self, point_clouds):
        ...
        global_features = F.adaptive_avg_pool1d(enc_features.permute(1, 2, 0), 1).squeeze(-1)
        ...
        return enc_xyz, enc_features, enc_inds, global_features, pre_enc_xyz
    ...
    def forward(self, inputs, is_eval: bool=False):
        ...
        enc_xyz, enc_features, enc_inds, global_features, pre_enc_xyz = self.run_encoder(point_clouds)
        enc_features = enc_features.permute(1, 2, 0)

        #diffusion
        if self.is_pretrain is True:
            batch, npoints, _ = pre_enc_xyz.shape
            shift = pre_enc_xyz.mean(dim=1, keepdim=True)
            scale = pre_enc_xyz.flatten(1).std(dim=1, keepdim=True).reshape(batch, 1, 1) / 0.75
            decoded_point_clouds = self.diffusion(global_features, npoints=npoints)
            decoded_point_clouds = decoded_point_clouds * scale + shift
        ...
        if self.criterion is not None and is_eval is False:
            if self.is_pretrain is True:
                (
                    ...
                    _
                ) = self.criterion(query_outputs, box_predictions, inputs, decoded_point_clouds, pre_enc_xyz)
            else:
                (
                    box_predictions['outputs']['assignments'], 
                    box_predictions['outputs']['loss'], 
...
def detector(args, dataset_config):
    ...
    criterion = build_criterion(cfg, dataset_config, is_pretrain=is_pretrain)
