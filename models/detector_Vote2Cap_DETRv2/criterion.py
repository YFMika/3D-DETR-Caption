...
class SetPredictionCriterion(nn.Module):
    def __init__(self, matcher, dataset_config, loss_weight_dict, is_pretrain):
        ...
        if is_pretrain is True:
            self.loss_functions = {
                "loss_sem_cls": self.loss_sem_cls_pre,
    ...
    def loss_sem_cls_pre(self, outputs, targets, assignments):
    ...
    def loss_sem_cls(self, outputs, targets, assignments):
        ...
        giou = torch.gather(
            outputs["gious"], 2, assignments["per_prop_gt_inds"].unsqueeze(-1)
        ).squeeze(-1).detach()
        giou = giou[gt_box_label != bg_class]
        pred_logits = pred_logits[gt_box_label != bg_class]
        gt_box_label = gt_box_label[gt_box_label != bg_class]
        
        src_score = torch.sigmoid(pred_logits.detach())
        target = F.one_hot(gt_box_label, num_classes=bg_class + 1)
        target_score_o = torch.zeros_like(gt_box_label, dtype=src_score.dtype)
        target_score_o = target_score_o + torch.relu(giou)
        target_score_o = target_score_o.unsqueeze(-1)

        target_score = target_score_o * target
...
class RebuildCriterion(nn.Module):
    
    def __init__(self, cfgs):
        super(RebuildCriterion, self).__init__()
        self.loss_dict = {
            'loss_rebuild': (self.loss_rebuild, 1),
        }

    
    def loss_rebuild(self, rebuild, origin) -> Dict:
        """ Compute vote loss: Match rebuild points to origin points.
    
        Args:
            encoded_features: Tensor (batch, npoints, 3)
            origin_features: Tensor (batch, npoints, 3)
        
        Returns:
            rebuild_loss: scalar Tensor
        """
    
        dist = torch.cdist(rebuild, origin, p=1)
        dist1, _ = torch.min(dist, dim=-1)
        dist2, _ = torch.min(dist, dim=-2)

        loss = torch.mean(dist1) + torch.mean(dist2)
        
        return {'loss_rebuild': loss}

    
    def forward(self, rebuild, origin) -> Dict:
        # assignments = self.compute_label_assignment(outputs, targets)
        
        loss = torch.zeros(1)[0].to(rebuild.device)
        loss_dict = {}
        loss_intermidiate = {}
        for loss_name, (loss_fn, loss_weight) in self.loss_dict.items():
            
            # loss_intermidiate = loss_fn(outputs, targets, assignments)
            loss_intermidiate = loss_fn(rebuild, origin)
            loss_dict.update(loss_intermidiate)
...
class OverallCriterion(nn.Module):
    def __init__(self, args, dataset_config, is_pretrain):
        ...
        self.set_prediction_loss = SetPredictionCriterion(
            matcher, dataset_config, loss_weight_dict, is_pretrain=is_pretrain
        )
    ...
    def forward(self, votenet_outputs: Dict, decoder_outputs: Dict, targets: Dict, rebuild, origin) -> Dict:
        ...
        if rebuild is not None:
            ...
            loss_dict.update(rebuild_loss_dict)
        else:
            rebuild_loss = 0
        ...
        return assignments, rebuild_loss + votenet_loss + set_loss, loss_dict
    

def build_criterion(args, dataset_config, is_pretrain):
    return OverallCriterion(args, dataset_config, is_pretrain)
